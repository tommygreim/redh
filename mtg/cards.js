/* ============================================================
 * Card database for the Secrets of Strixhaven MVP client.
 * The raw CSV is embedded verbatim and parsed at runtime so the
 * data stays byte-identical to the source list.
 * ============================================================ */

const CARD_CSV = `name,mana_cost,type_line,oracle_text,power,toughness
...
/* Oracle text for the well-known reprints that appear as prepare
 * spells (the CSV leaves split-card oracle text blank). Custom
 * prepare spells whose text wasn't provided stay blank. */
const PREPARE_TEXTS = {
  ...
  'Demonic Tutor': 'Search your library for a card, put that card into your hand, then shuffle.',
  ...
  'Reanimate': 'Put target creature card from a graveyard onto the battlefield under your control. You lose life equal to its mana value.',
...
function manaValue(cost) {
  ...
  for (const sym of cost.match(/\{[^}]+\}/g) || []) {
    ...
    else if (/^2\/[WUBRG]$/.test(inner)) mv += 2;
    else mv += 1; // colored, hybrid, phyrexian
  }
  return mv;
}

function colorsOf(cost) {
  const colors = new Set();
  for (const sym of (cost || '').match(/\{[^}]+\}/g) || []) {
    for (const ch of sym.slice(1, -1)) {
      if ('WUBRG'.includes(ch)) colors.add(ch);
    }
  }
  return [...'WUBRG'].filter(c => colors.has(c));
}

/* Extract simple tap-for-mana options from oracle text.
 * Returns a list of options, each a string of mana symbols, e.g.
 * "Add {B} or {G}" -> ['B','G'];  "Add {R}{R}" -> ['RR'];
 * "Add one mana of any color" -> ['W','U','B','R','G']. */
function manaOptions(text) {
  if (!text) return null;
  const m = text.match(/\{T\}(?:, [^:]*)?: Add ([^.\n]+)/);
  if (!m) return null;
  const clause = m[1];
  if (/one mana of any color/.test(clause)) return ['W', 'U', 'B', 'R', 'G'];
  const opts = [];
  for (const part of clause.split(' or ')) {
    const syms = (part.match(/\{([WUBRGC])\}/g) || []).map(s => s[1]);
    if (syms.length) opts.push(syms.join(''));
  }
...
function buildDB() {
  ...
  for (const r of rows) {
    ...
    const [name, cost, typeLine, text, power, toughness] = r;
    const split = name.includes(' // ');
    const card = {
      name: split ? name.split(' // ')[0] : name,
      fullName: name,
      cost: split ? (cost.split(' // ')[0] || '').trim() : cost,
      typeLine: split ? typeLine.split(' // ')[0].trim() : typeLine,
      text: text || '',
    ...
    if (split) {
      const spellName = name.split(' // ')[1];
      card.prepare = {
        name: spellName,
        cost: (cost.split(' // ')[1] || '').trim(),
        typeLine: (typeLine.split(' // ')[1] || '').trim(),
        text: PREPARE_TEXTS[spellName] || '(Spell text not in database — resolve manually.)',
      };
      if (!card.text) {
        card.text = `Prepare — ${spellName} ${card.prepare.cost} (${card.prepare.typeLine}). ` +
          `While this creature is on the battlefield and prepared, you may cast ${spellName}. ` +
          `It becomes unprepared when you do.`;
      }
    }
    card.mv = manaValue(card.cost);
    card.colors = colorsOf(card.cost);
    card.isLand = /\bLand\b/.test(card.typeLine);
    card.isCreature = /\bCreature\b/.test(card.typeLine);
    card.isInstant = /\bInstant\b/.test(card.typeLine);
    card.isSorcery = /\bSorcery\b/.test(card.typeLine);
    card.isPermanent = !card.isInstant && !card.isSorcery;
    card.manaOpts = manaOptions(card.text);
    db[card.fullName] = card;
  }
  return db;
}

const CARD_DB = buildDB();
const CARD_NAMES = Object.keys(CARD_DB).sort();

/* Lookup that accepts the full "A // B" name, the front-face name,
 * or any case-insensitive match of either. */
const NAME_INDEX = (() => {
  const idx = {};
  for (const full of Object.keys(CARD_DB)) {
    idx[full.toLowerCase()] = full;
    idx[CARD_DB[full].name.toLowerCase()] = full;
  }
  return idx;
})();

function findCard(name) {
  const key = NAME_INDEX[name.trim().toLowerCase()];
  return key ? CARD_DB[key] : null;
}

/* Predefined tokens referenced by cards in this set. */
const TOKEN_DB = [
  { name: 'Fractal', cost: '', typeLine: 'Token Creature — Fractal', text: '', power: '0', toughness: '0', colors: ['U', 'G'] },
  { name: 'Pest', cost: '', typeLine: 'Token Creature — Pest', text: 'Whenever this token attacks, you gain 1 life.', power: '1', toughness: '1', colors: ['B', 'G'] },
  { name: 'Inkling', cost: '', typeLine: 'Token Creature — Inkling', text: 'Flying', power: '1', toughness: '1', colors: ['W', 'B'] },
  { name: 'Spirit', cost: '', typeLine: 'Token Creature — Spirit', text: '', power: '2', toughness: '2', colors: ['R', 'W'] },
  { name: 'Elemental', cost: '', typeLine: 'Token Creature — Elemental', text: 'Flying', power: '3', toughness: '3', colors: ['U', 'R'] },
  { name: 'Treasure', cost: '', typeLine: 'Token Artifact — Treasure', text: '{T}, Sacrifice this token: Add one mana of any color.', power: null, toughness: null, colors: [] },
  { name: 'Clue', cost: '', typeLine: 'Token Artifact — Clue', text: '{2}, Sacrifice this token: Draw a card.', power: null, toughness: null, colors: [] },
  { name: 'Goblin', cost: '', typeLine: 'Token Creature — Goblin', text: '', power: '1', toughness: '1', colors: ['R'] },
  { name: 'Rabbit', cost: '', typeLine: 'Token Creature — Rabbit', text: '', power: '1', toughness: '1', colors: ['W'] },
  { name: 'Forest Dryad', cost: '', typeLine: 'Token Land Creature — Forest Dryad', text: '({T}: Add {G}.)', power: '1', toughness: '1', colors: ['G'] },
  { name: 'Ape', cost: '', typeLine: 'Token Creature — Ape', text: '', power: '3', toughness: '3', colors: ['G'] },
  { name: 'Lizard', cost: '', typeLine: 'Token Creature — Lizard', text: '', power: '8', toughness: '8', colors: ['R'] },
  { name: 'Monster Role', cost: '', typeLine: 'Token Enchantment — Aura Role', text: 'Enchanted creature gets +1/+1 and has trample.', power: null, toughness: null, colors: [] },
  { name: 'Royal Role', cost: '', typeLine: 'Token Enchantment — Aura Role', text: 'Enchanted creature gets +1/+1 and has ward {1}.', power: null, toughness: null, colors: [] },
];

for (const t of TOKEN_DB) {
  t.fullName = t.name;
  t.mv = 0;
  t.isLand = /\bLand\b/.test(t.typeLine);
  t.isCreature = /\bCreature\b/.test(t.typeLine);
  t.isInstant = false;
  t.isSorcery = false;
  t.isPermanent = true;
  t.prepare = null;
  t.manaOpts = manaOptions(t.text);
  t.isToken = true;
}

if (typeof module !== 'undefined') {
  module.exports = { CARD_DB, CARD_NAMES, TOKEN_DB, findCard, manaValue, colorsOf, manaOptions };
}
